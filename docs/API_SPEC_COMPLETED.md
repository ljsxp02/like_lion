...
- 실제 JWT/Spring Security 인증 및 역할별 권한 처리
...
관련 API 명세와 Controller/Service 골격은 추후 확장을 위해 남겨둘 수 있으나, 현재 구현 완료 API로 간주하지 않는다. 로그인 사용자가 필요한 기능은 현재 `MockCurrentUserProvider` 기반으로 동작한다. 업종 카테고리용 DB 구조는 추가하지 않으며 지도용 매장 응답의 `categories`는 빈 배열을 반환한다.
...
| Auth | 로그인 | POST | `/api/v1/auth/login` | No | 명세 완료 |
...
| QR/Benefit Usage | QR 인증 요청 | POST | `/api/v1/qr/verify` | Yes | 추후 개발 예정 |
| QR/Benefit Usage | 내 혜택 사용 내역 조회 | GET | `/api/v1/me/benefit-usages` | Yes | 명세 완료 |
| Admin | 관리자 매장 등록 | POST | `/api/v1/admin/stores` | Admin | 추후 개발 예정 |
| Admin | 관리자 매장 수정 | PATCH | `/api/v1/admin/stores/{storeId}` | Admin | 추후 개발 예정 |
| Admin | 관리자 매장 비활성화 | DELETE | `/api/v1/admin/stores/{storeId}` | Admin | 추후 개발 예정 |
| Admin | 관리자 QR 토큰 재발급 | POST | `/api/v1/admin/stores/{storeId}/qr/regenerate` | Admin | 추후 개발 예정 |
| Admin | 관리자 메뉴 등록 | POST | `/api/v1/admin/stores/{storeId}/menus` | Admin | 추후 개발 예정 |
| Admin | 관리자 메뉴 수정 | PATCH | `/api/v1/admin/menus/{menuId}` | Admin | 추후 개발 예정 |
| Admin | 관리자 혜택 등록 | POST | `/api/v1/admin/stores/{storeId}/benefits` | Admin | 추후 개발 예정 |
| Admin | 관리자 혜택 수정 | PATCH | `/api/v1/admin/benefits/{benefitId}` | Admin | 추후 개발 예정 |
...
| password | String | Y | 비밀번호 원문. 현재 시연 버전은 원문 비교 | `Password123!` |
...
회원가입 후 `/api/v1/auth/login`을 호출해 accessToken과 refreshToken을 발급받는다.
...
추후 개발 예정 API다. 명세는 확정하되 실제 구현 우선순위에서는 제외한다.
...
추후 개발 예정 API다. 관리자 권한 정책과 요청/응답 구조만 먼저 확정한다.
...
추후 개발 예정 API다. 매장 삭제는 소프트 삭제로 처리한다. 소프트 삭제는 DB row를 물리적으로 제거하지 않고 `isActive=false` 또는 `deletedAt` 값을 기록해 일반 사용자 조회에서만 숨기는 방식이다. 과거 혜택 사용 내역, 즐겨찾기, QR 인증 기록과의 연결을 유지할 수 있어 매장 데이터에는 이 방식이 적합하다.
...
| 로그인/회원가입 토큰 발급 | 회원가입 완료와 로그인 모두 access/refresh token 반환 |
...
| QR 인증 API | 인증 필수, 추후 개발 예정 |
| 관리자 API | 관리자 권한 필수, 추후 개발 예정 |
